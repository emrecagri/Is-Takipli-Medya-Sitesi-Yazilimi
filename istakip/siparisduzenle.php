							...
							<div class="form-row">	
								...
								<div class="form-group col-md-6">
									...
									<select id="inputState" name="sip_durum" class="form-control">
										<option <?php if($durum == 'Sipariş Onaylandı'){echo("selected");}?> value="Sipariş Onaylandı">Sipariş Onaylandı</option>
										<option <?php if($durum == 'Sipariş &Uuml;zerinde &Ccedil;alışılıyor'){echo("selected");}?> value="Sipariş Üzerinde Çalışılıyor">Sipariş Üzerinde Çalışılıyor</option>
										<option <?php if($durum == 'Sipariş Tamamlandı'){echo("selected");}?> value="Sipariş Tamamlandı">Sipariş Tamamlandı</option>
									</select>
								</div>



								<?php $odeme=$kayitcek['sip_odeme']; 
								$odemedurumu = $odeme;
								?>
								<div class="form-group col-md-6">
									<label>Ödeme Durumu</label>
									<select id="inputState" name="sip_odeme" class="form-control">
										<option <?php if($odemedurumu == '&Ouml;dendi'){echo("selected");}?> value="Ödendi">Ödendi</option>
										<option <?php if($odemedurumu == '&Ouml;denmedi'){echo("selected");}?> value="Ödenmedi">Ödenmedi</option>
									</select>
								</div>
